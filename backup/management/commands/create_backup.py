...
from backup.drive_utils import upload_to_dropbox
from backup.models import Backup

default_apps = ['bot']

class Command(BaseCommand):
    help = "Backup apps, DB, media, and upload to Dropbox"
    ...
    def handle(self, *args, **options):
        ...
        with zipfile.ZipFile(backup_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            ...
            for app in apps_to_backup:
                ...
                if os.path.isdir(app_path):
                    ...
                    self.stdout.write(f"⚠️ App folder not found: {app}")
        ...
        if missing:
            self.stdout.write(f"❌ Missing files: {missing}")
        else:
            self.stdout.write("✅ All expected files are present in the backup zip.")

        # Upload to Dropbox
        try:
            uploaded_file = upload_to_dropbox(backup_path)
            backup_size = os.path.getsize(backup_path)

            Backup.objects.create(
                file_name=os.path.basename(backup_path),
                drive_url=uploaded_file.get('shared_link'),  # ✅ Dropbox shared link
                backup_size=backup_size
            )

            self.stdout.write(f"☁️ Uploaded to Dropbox: {uploaded_file.get('shared_link')}")
        except Exception as e:
            self.stdout.write(f"❌ Dropbox upload failed: {e}")
